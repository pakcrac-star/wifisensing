...
static void channel_hop_task(void *pvParameters)
{
    ...
    while (s_hal.is_started) {
        // SAFETY: never hop channels while connected to an AP
        if (is_passive_mode(s_hal.current_mode) && !s_hal.is_connected) {
            uint8_t next_chan = channels[channel_idx];
            esp_err_t r = esp_wifi_set_channel(next_chan, WIFI_SECOND_CHAN_NONE);
            if (r == ESP_OK) {
                s_hal.current_channel = next_chan;
                ESP_LOGI(TAG, "Hopped to channel %d", next_chan);
            } else {
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(TAG, "WIFI_EVENT id=%d (is_started=%d, current_mode=%d, is_connected=%d)",
             event_id, (int)s_hal.is_started, (int)s_hal.current_mode, (int)s_hal.is_connected);
    ...
    switch (event_id) {
        case WIFI_EVENT_STA_START: {
            ...
            if (s_hal.is_started && !is_passive_mode(s_hal.current_mode)) {
                ...
                ESP_LOGI(TAG, "esp_wifi_connect() -> %d", r);
            }
            break;
        }
        ...
        case WIFI_EVENT_STA_DISCONNECTED: {
            ...
            ESP_LOGW(TAG, "WIFI_EVENT_STA_DISCONNECTED reason=%d (is_started=%d, mode=%d)",
                     reason, (int)s_hal.is_started, (int)s_hal.current_mode);

            if (s_hal.is_started && !is_passive_mode(s_hal.current_mode)) {
                __atomic_fetch_add(&s_hal.stats.reconnect_count, 1, __ATOMIC_RELAXED);
                esp_err_t r = esp_wifi_connect();
                ESP_LOGI(TAG, "Reconnecting: esp_wifi_connect() -> %d", r);
            }
            break;
        }
...
// after esp_wifi_set_ps
ret = esp_wifi_set_ps(WIFI_PS_NONE);
ESP_LOGI(TAG, "esp_wifi_set_ps(WIFI_PS_NONE) -> %d", ret);
if (ret != ESP_OK) return ret;

// when enabling promiscuous (if used)
    if (is_passive_mode(s_hal.current_mode)) {
        wifi_promiscuous_filter_t filter = {
            .filter_mask = WIFI_PROMIS_FILTER_MASK_MGMT | WIFI_PROMIS_FILTER_MASK_DATA
        };
        ret = esp_wifi_set_promiscuous_filter(&filter);
        ESP_LOGI(TAG, "esp_wifi_set_promiscuous_filter -> %d", ret);
        if (ret != ESP_OK) return ret;

        ret = esp_wifi_set_promiscuous_rx_cb(&wifi_promiscuous_cb);
        ESP_LOGI(TAG, "esp_wifi_set_promiscuous_rx_cb -> %d", ret);
        if (ret != ESP_OK) return ret;
    ...
    /* --------------------------------------------------------
     * 4. Configure & Enable CSI Sampling (declare csi_config here)
     * -------------------------------------------------------- */
    ...
    ESP_LOGI(TAG, "esp_wifi_set_csi_config -> %d", ret);
    if (ret != ESP_OK) return ret;

    ret = esp_wifi_set_csi_rx_cb(&wifi_csi_cb, NULL);
    ESP_LOGI(TAG, "esp_wifi_set_csi_rx_cb -> %d", ret);
    if (ret != ESP_OK) return ret;

    ret = esp_wifi_set_csi(true);
    ESP_LOGI(TAG, "esp_wifi_set_csi -> %d", ret);
    if (ret != ESP_OK) return ret;
    return ESP_OK;
}
