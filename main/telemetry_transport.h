 ...
 *
 * Note: This symbol is allowed to be a weak optional implementation. Callers may
 * check the function pointer at runtime before invoking to support optional
 * link-time providers.
 */
int telemetry_transport_send(const void *payload, size_t size) __attribute__((weak));
